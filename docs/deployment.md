...
| Hostname                 | Port | Path |  |
|--------------------------|------|------|---------|
| team8.local              | 80   | /    | SMS Checker App Frontend |
| team8.local              | 80   | /sms | Proxy serves request to App Service |
| team8.local              | 80   | /sms/metrics | App-service metrics endpoint |
| canary.team8.local       | 80   | /    | Prereleased app (canary version) used for experimentation |
| prometheus.team8.local   | 80   | /    | Prometheus web UI for metrics inspection and alerting |
| grafana.team8.local      | 80   | /    | Grafana web UI for dashboarding |
...
- Referenced by VirtualService resources (my-vs, canary-vs, grafana-vs, prometheus-vs)
...
### 9. VirtualService (canary-vs)
**Name:**  canary-vs
**Type:**  VirtualService
**Description:**  It routes traffic from the canary hostname (`canary.team8.local`) directly to the canary (v2) versions, bypassing the 90/10 split. All requests to `/sms` are routed to app-service v2, and all other requests are routed to app-frontend v2.
**Connections:**
- Routes to Service (app-frontend, app-service) subset canary (v2) based on the DestinationRule for app-frontend and app-service.

### 10. VirtualService (app-service-vs)
**Name:**  app-service-vs   
**Type:**  VirtualService   
...
### 11. DestinationRule (app-service-dr)
**Name:**  app-service-dr      
**Type:**  DestinationRule      
...
### 14. Deployment (app-service-v2)
**Name:**  app-service-v2    
**Type:**  Deployment    
**Description:** Creates the pods that run the canary version of the app service which are injected with an Istio sidecar proxy and are labeled with version v2. It receives 10% of the traffic. This instance is a prerelease of app service implementing the experiment.      
**Connections:**
- Receives traffic from App Frontend.

### 15. VirtualService (model-service-vs)
...
### 16. DestinationRule (model-service-dr)
...
### 18. Deployment (model-service-v1)
...
### 19. Deployment (model-service-v2)
...
### 20. Deployment (model-service-v3)
...
### Grafana
---
### 21. VirtualService (grafana-vs)
**Name:**  grafana-vs    
**Type:**  VirtualService    
**Description:**  It routes traffic from the Gateway to the Grafana service.   
**Connections:**
- Routes to Service `team8-app-grafana.default.svc.cluster.local`. 

### 22. ConfigMap (grafana-dashboards-sms-app)
...
### Prometheus
---
### 23. VirtualService (prometheus-vs)
**Name:**  prometheus-vs
**Type:**  VirtualService
**Description:**  It routes traffic from the Gateway to the Prometheus service.     
**Connections:**  
- Routes to Service `team8-app-kube-prometheus-prometheus.default.svc.cluster.local`.

### 24. ServiceMonitor (mymonitor)
**Name:**  mymonitor   
**Type:**  ServiceMonitor  
**Description:**  It is defined to make the app-service discoverable by Prometheus such that it can scrape the metrics defined at the `/sms/metrics` endpoint in the app-service.  
**Connections:**
- It watches app-service and its endpoints.

### 25. PrometheusRule (sms-app-alert)
**Name:**  sms-app-alert   
**Type:**  PrometheusRule  
...
### 27. AlertmanagerConfig (email-config)
**Name:**  email-config   
...
### 28. Secret (alertmanager-smtp-secret)
...
The diagram above describes the flow of an http call to http://team8.local/sms . The call is picked up by the gateway and the virtual service assigns it to an app-service pod. If the cookie "canary-user" has value true, the call is routed to the canary app-service, otherwise the default route points to the stable app-service. The virtual service `model-service-vs` routes calls that pass through app-service v1 to the stable model and the ones that pass through app-service v2 to the canary release. 100% of the traffic routed to model v1 and model v2 is mirrored to the shadow launch model v3.
...
Monitoring of the kubernetes stack provides observability of the behaviour of the application and enables engineers to make informed decisions by supporting continuous experimentation. Monitoring is implemented using Prometheus, Grafana and an Alert Manager. Prometheus and Grafana are added directly by specifying the kube-prometheus-stack as a dependency in the helm chart.

**Prometheus** scrapes available app-services for metrics, every 15 seconds. These metrics are then passed to Grafana for visualization and evaluated against the alert rule for alert firing.

**Grafana** provides visualization for the app metrics through dashboards. The application contains 2 dashboards, one for visualizing the app metrics and another for visualizing quantitative evidence of the experiment.
